...
class LoginForm(forms.Form):
    ...
    def clean(self):
        ...
        input_captcha = self.cleaned_data.get('captcha', '').upper()

        if input_captcha == '':
            raise forms.ValidationError('验证码不能为空')
        else:
            captcha = self.request.session.get('captcha', '').upper()
...
class ForgotPasswordForm(forms.Form):
    ...
    def clean_verification_code(self):
        ...
        # 判断验证码
        code = self.request.session.get('forgot_password_code', '').upper()
